...
def model_fn(model_dir):
    ...
    if os.path.exists(class_to_idx_path):
        with open(class_to_idx_path, "r") as f:
            ...
            print(f"Svetlana Loaded idx_to_class mapping: {model.idx_to_class}")
    else:
        print(f"Svetlana Error: {class_to_idx_path} not found.")
...
def input_fn(request_body, request_content_type):
    try:
        print(f"Svetlana request_content_type: {request_content_type}")
        if request_content_type == "application/json":
            ...
            print("Svetlana Processing binary image data")
        ...
        print("Svetlana Image successfully loaded and processed")
        ...
        print(f"Svetlana Error in input_fn: {e}")
        raise
...
def predict_fn(input_data, model):
    try:
        ...
        print(f"IDK  Error in predict_fn: {e}")
        raise
...
def output_fn(prediction, content_type='application/json'):
    try:
        if content_type == 'application/json':
            ...
            print("Svetlana Output formatted successfully.")
            return response
        else:
            raise ValueError(f"Unsupported content type: {content_type}")
    except Exception as e:
        print(f"IDK Error in output_fn: {e}")
        raise